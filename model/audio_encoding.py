...
class CNN2D(Module):
	...
	def conv_block(self, in_channels: int, out_channels: int, conv_kernel: int, conv_stride: int, pool_kernel: int=0) -> Module:
		block = [Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=conv_kernel, stride=conv_stride), BatchNorm2d(num_features=out_channels)]
		if pool_kernel:
...
def audio_example():
	...
	mel_freqs = torch.stack([tf.MelSpectrogram(sample_rate)(wav).to(device) for wav, sample_rate in zip(wavs, sample_rates)])
