...
def train_batch(model: Module, X: FloatTensor, Y: LongTensor, loss_fn: tensor_map, opt: Optimizer) -> float:
...
def train_epoch(model: Module, dl: DataLoader, loss_fn: tensor_map, opt: Optimizer, device: str) -> float:
	epoch_loss = 0.
...
def train_model(model: Module, train_dl: DataLoader, val_dl: DataLoader, loss_fn: tensor_map, 
				opt: Optimizer, num_epochs: int, device: str) -> List[Tuple[float,float]]:
	losses = []
	print('\nStarted training..')
